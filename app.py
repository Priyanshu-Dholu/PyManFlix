...
class Movies(db.Model):
    ...
    def __repr__(self) -> str:
        return f"movies('{self.title}','{self.year}','{self.link}','{self.added_on},{self.quality}','{self.dolby_audio}','{self.dual_audio},{self.poster_link},{self.movie_id_tmd}')"
...
def hello_world():
    if request.method == 'POST': 
        ...
        movies = Movies(title=title, year=year, link=link, quality=quality,
                        dolby_audio=dolby_audio, dual_audio=dual_audio, poster_link=poster_link, movie_id_tmd=movie_id_tmd)
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        quality = request.form.get('quality')
        if quality == None or quality == False:
            quality = True
        elif quality == True:
            quality = False
        db.session.commit()
